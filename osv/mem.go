...
func memStat(ns core.Namespace, swagURL string) (*plugin.MetricType, error) {
	memType := ns[2].Value
	switch {
	case regexp.MustCompile(`^/osv/memory/free`).MatchString(ns.String()):
	...
	case regexp.MustCompile(`^/osv/memory/total`).MatchString(ns.String()):
		metric, err := getMemStat(swagURL, memType)
		if err != nil {
			return nil, err
		}
...
func getMemoryMetricTypes() ([]plugin.MetricType, error) {
	...
	for _, metricType := range memMetrics {
		mts = append(mts, plugin.MetricType{Namespace_: core.NewNamespace("osv", "memory", metricType.Value)})
	}
	return mts, nil
}
